...
export const AntiGravityCanvas: React.FC<AntiGravityCanvasProps> = ({ invert = false }) => {
  ...
  const initParticles = useCallback((width: number, height: number) => {
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      newParticles.push({
        ...
        color: Math.random() > 0.9 ? '#4285F4' : (invert ? '#0f172a' : '#ffffff'),
  ...
  // Global mouse tracking: listens on window so mousemove is captured even when
  // elements with higher z-index (forms, text blocks) sit on top of the canvas.
  // Coordinates are converted to be relative to the container on every move.
  useEffect(() => {
  ...
  return (
    <div
      ref={containerRef}
      className={`absolute inset-0 z-0 overflow-hidden ${invert ? "bg-white" : "bg-black"}`}
    >
      <canvas ref={canvasRef} className="block w-full h-full" />

      {/* Debug Info Overlay (Hidden in production usually, but cool for tech demos) */}
      <div className={`absolute bottom-4 right-4 pointer-events-none text-xs font-mono text-right ${invert ? "text-black/20" : "text-white/20"}`}>
